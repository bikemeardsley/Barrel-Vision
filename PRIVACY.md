...
- **Fetches public baseball data** with anonymous `GET` requests to:
  ...
  - `pitcherlist.com` (the public weekly starting-pitcher and reliever ranking articles — only the
    factual rank/name/team/tier are read; the written analysis is not stored)
...
- It has **no backend server**. The only network requests are the three public data sources above.
...
- Host access to `baseballsavant.mlb.com`, `statsapi.mlb.com`, and `pitcherlist.com` — to fetch the
  public stats and rankings shown in the overlay. The extension requests no access to ESPN or any other
  site's data.
...
Barrel Vision is not affiliated with, endorsed by, or sponsored by ESPN, MLB, or Pitcher List. "Baseball
Savant" and "Statcast" are properties of MLB Advanced Media.
